...
from auth_api.modules.users.user_entity import MfaRecoveryCodeEntity, UserEntity
...
class MfaService:
    ...
    def disable(self, subject: str, code: str) -> None:
        with SessionLocal.begin() as database:
            user = self._user(database, subject)
    ...
    def verify_user_code(self, database, user: UserEntity, code: str) -> bool:
        ...
        if user.mfa_secret_ciphertext and self._verify_totp(
            decrypt_secret(user.mfa_secret_ciphertext),
            normalized,
        ):
            return True
