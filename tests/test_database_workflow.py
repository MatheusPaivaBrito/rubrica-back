...
def test_database_workflow_round_trip(tmp_path: Path) -> None:
    ...
    try:
        ...
        invitation = service.add_signer(request.id, SignerCreate(auth_user_id="database-user", name="Database User", email="database@example.com"), "operator")
        service.open_request(request.id, "operator")
        service.sign(invitation.signing_token, "database-user", True)
